...
class MazeGenerator:
    ...
    def generate(self) -> Maze:
        ...
        if not self.perfect:
            self._add_loops(maze, reserved)
            self._braid_dead_ends(maze, reserved)
            self._prevent_open_areas(maze)  # <-- 新增修补防护
        ...
        for top_y in range(self.height - 2):
            for left_x in range(self.width - 2):
                if self._is_open_3x3(maze, left_x, top_y):
                    # 发现 3x3 纯空旷区域，在中心点与相邻点之间加回一堵内墙
                    center: Position = (left_x + 1, top_y + 1)
                    # 优先在中心点与其北侧格子之间重新砌墙
                    north_neighbour: Position = (center[0], center[1] - 1)
                    maze.seal(center, north_neighbour)
...
---------------------------------------------
    ...
    def _add_loops(
        ...
        # 打通若干道内墙增加独立回路
        for first, second in candidates[:extra_loops_target]:
        ...
        for _ in range(5):
            ...
            for pos in dead_ends:
                ...
                carve_candidates = [
                    ...
                    if neighbour not in reserved
                    and not maze.is_open(pos, direction)
                ]

                if carve_candidates:
                    chosen = self._random.choice(carve_candidates)
